...
namespace :gke do
  ...
  task :deploy do
    ...
    prompt_branch_commit unless fetch(:image_tag)
...
def apply_kubernetes_resources
  run :local do
    ...
    command "REVISION=#{fetch(:image_tag)} KUBECONFIG=#{fetch(:kube_config)} ENVIRONMENT=#{fetch(:stage)} kubernetes-deploy --bindings=image_repo=#{fetch(:image_repo)},namespace=#{fetch(:namespace)} #{fetch(:namespace)} #{fetch(:context)}"
  end
end
